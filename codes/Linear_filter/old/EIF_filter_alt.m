function [mu,rho,P_LF,cv,dbl_count] =...
    EIF_filter(gamma,lambda,sigma,N,alg,lnl)
...
n_max=400; % 200Hz upper and lower limits for non-linearity.
%D0 = sum(real(1000*A_t((bins/2):end)))*dt;
D0 = sum(real(1000*A_t))*dt;
nonlinear = zeros(n_max,1);
E_span = linspace(-500,500,n_max);
for i=1:n_max
    % Changed sigmap -> sigma_pert. Not sure if this is justified. It
    % seems to work though!
    nonlinear(i) = calc_Rate_cuEIF(E0+E_span(i)/r0_prime_E0,...
