function [mu,rho,P_LF,cv,dbl_count] =...
    EIF_filter_nl_change(gamma,lambda,sigma,N,alg,lnl,bin_size,normed)
...
r_est = conv(I_common,real(A_t),'same')*dt; % Linear firing rate.
r_nl = r0 + interp1(E_span,nonlinear,r_est,'spline'); % LNL Cascade.
r_nl_full = r0 + interp1(E_span,nonlinear_full,r_est,'spline'); % LNL Cascade.

% Generate spikes using the linear or non-linear firing rate:
if strcmp(normed,'none')
    if strcmp(lnl,'none')
        [mu,rho,P_LF,cv,dbl_count,~,~] =...
        Spikes_from_Lin_Filter_no_norm(r_est,bin_size,N,alg,dt,num_of_bins);
    elseif strcmp(lnl,'l')
        r_est=max(0,r_est);
        [mu,rho,P_LF,cv,dbl_count,~,~] =...
        Spikes_from_Lin_Filter_no_norm(r_est,bin_size,N,alg,dt,num_of_bins);
    elseif strcmp(lnl,'lnl')
        [mu,rho,P_LF,cv,dbl_count,~,~] =...
        Spikes_from_Lin_Filter_no_norm(r_nl,bin_size,N,alg,dt,num_of_bins);
    elseif strcmp(lnl,'lnl_full')
        [mu,rho,P_LF,cv,dbl_count,~,~] =...
        Spikes_from_Lin_Filter_no_norm(r_nl_full,bin_size,N,alg,dt,num_of_bins);
    end
elseif strcmp(normed,'normed')
    if strcmp(lnl,'none')
        [mu,rho,P_LF,cv,dbl_count,~,~] =...
