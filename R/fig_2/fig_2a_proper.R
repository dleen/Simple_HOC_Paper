...
p2b <- p2b + xlab("Population size N") + ylab(expression("JS-div / log N   "* (10^{-2})*""))
...
p2b <- p2b + scale_y_continuous(limits=c(0,8e-02),breaks=c(0,2e-02,4e-02,6e-02,8e-02),labels=c(0,2,4,6,8))
...
p2b <- p2b + opts(legend.position = c(0.33, 0.86)) + opts(legend.text = theme_text(size = 9), legend.key.width = unit(0.5,"cm"), legend.key.height = unit(0.25, "cm"))
...
p2c <- p2c + scale_x_continuous(limits = c(4, 100), breaks = c(4, 25, 50, 75, 99.5), labels = c(4, 25, 50, 75, 100))
p2c <- p2c + scale_y_continuous(limits=c(0,3e-02),breaks=c(0,1e-02,2e-02,3e-02),labels=c(0,1,2,3))
