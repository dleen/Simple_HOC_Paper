...
p2 <- p2 + scale_y_log10(limits = c(1e-05, 1), breaks = c(0.1, 0.001, 1e-05), labels = c(expression(10^-1), expression(10^-3), expression(10^-5)))
...
p2_1 <- p2_1 + scale_y_log10(limits = c(1e-05, 1), breaks = c(0.1, 0.001, 1e-05), labels = c(expression(10^-1), expression(10^-3), expression(10^-5)))
...
p2_2 <- p2_2 + scale_y_log10(limits = c(1e-05, 1), breaks = c(0.1, 0.001, 1e-05), labels = c(expression(10^-1), expression(10^-3), expression(10^-5)))
...
fig_2b <- readMat("/Users/dleen/Dropbox/Research/Simple_HOC_draft/codes/Figure_code/fig_2b_R_logN_DJS.mat")
...
#p2b <- p2b + scale_color_gradient("", breaks = c(0.05, 0.1, 0.25), labels = c(expression("" * rho * " = 0.05"), expression("" * rho * " = 0.1  "), expression("" * rho * " = 0.25")))
p2b <- p2b + xlab("Population size N") + ylab("JS-div / log N")
#p2b <- p2b + coord_cartesian(x = c(0, 100), y = c(-0.02, 1))
#p2b <- p2b + scale_y_continuous(limits = c(-0.02, 1), breaks = c(0.25,0.5,0.75,1), labels = c("0.25","0.5","0.75","1.0"))
#p2b <- p2b + coord_cartesian(x = c(0, 100))
#p2b <- p2b + scale_y_continuous(limits = c(-0.02, 1), breaks = c(0.25,0.5,0.75,1), labels = c("0.25","0.5","0.75","1.0"))
p2b <- p2b + opts(legend.key = theme_blank())
p2b <- p2b + scale_x_continuous(limits = c(0, 100), breaks = c(0.1, 25, 50, 75, 99.9), labels = c(0, 25, 50, 75, 100))
...
p2b <- p2b + opts(plot.margin = unit(c(0.25,0.2,0,0), "cm"))	
p2b <- p2b + opts(legend.position = c(0.40, 0.86)) + opts(legend.text = theme_text(size = 9), legend.key.width = unit(0.5,"cm"), legend.key.height = unit(0.25, "cm"))
p2b <- p2b + opts(axis.text.x = theme_text(size = 7))
p2b <- p2b + opts(axis.text.y = theme_text(size = 7))
p2b <- p2b + opts(axis.title.y = theme_text(angle = 90, vjust = 0.38, size = 8))
...
fig_2c <- readMat("/Users/dleen/Dropbox/Research/Simple_HOC_draft/codes/Figure_code/fig_2c_R_logN_DJS.mat")
...
#p2c <- p2c + scale_color_gradient("", breaks = c(0.1, 0.2, 0.3), labels = c(expression("" * mu * " = 0.1"), expression("" * mu * " = 0.2"), expression("" * mu * " = 0.3")))
#p2c <- p2c + xlab("Population size N") + ylab("KL-divergence / log N")
p2c <- p2c + xlab("Population size N") + ylab(NULL)
#p2c <- p2c + coord_cartesian(x = c(0, 100))
#p2c <- p2c + scale_y_continuous(limits = c(-0.02, 1), breaks = c(0.25,0.5,0.75,1), labels = c("0.25","0.5","0.75","1.0"))
#p2c <- p2c + coord_cartesian(x = c(0, 100), y = c(-0.02, 1))
#p2c <- p2c + scale_y_continuous(limits = c(-0.02, 1), breaks = c(0.25,0.5,0.75,1),labels=c("0.25","0.5","0.75","1.0"))
p2c <- p2c + opts(legend.key = theme_blank())
p2c <- p2c + scale_x_continuous(limits = c(0, 100), breaks = c(0, 25, 50, 75, 99.5), labels = c(0, 25, 50, 75, 100))
...
p2c <- p2c + opts(plot.margin = unit(c(0.25,0.2,0,0), "cm"))
p2c <- p2c + opts(legend.position = c(0.40, 0.86)) + opts(legend.text = theme_text(size = 9), legend.key.width = unit(0.5,"cm"), legend.key.height = unit(0.25, "cm"))
p2c <- p2c + opts(axis.text.x = theme_text(size = 7))
p2c <- p2c + opts(axis.text.y = theme_text(size = 7))
p2c <- p2c + opts(axis.title.y = theme_text(angle = 90, vjust = 0.2, size = 10))
